...
const JobstreetJobSalaryCard = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await Api.get("/data-visualization/job-salaries", {
        params,
      });

      const jobSalaryData = response.data.dataset.map((item) => ({
        label: item.salary,
        total: item.amount,
      }));
      setChartData(jobSalaryData);
    } catch (error) {
      console.error("Error fetching job salary data:", error);
      message.destroy();
      message.error("Oops! Something went wrong while loading jobstreet job salaries. Please try again later.");
      setChartData([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Card>
      <div className="flex justify-between items-center mb-4">
        <Text className="font-bold text-xl block mb-4">Job Salary</Text>
...
export default JobstreetJobSalaryCard;
